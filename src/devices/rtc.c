/*! \file rtc.c
 *
 * This code is an interface to the MC146818A-compatible real time clock
 * found on PC motherboards.  See [MC146818A] for hardware details.
 */
...
/*! Indexes of CMOS registers with real-time clock functions.  Note that all of
    these registers are in BCD format, so that 0x59 means 59, not 89. @{ */
...
/*! Returns number of seconds since Unix epoch of January 1, 1970. */
time_t rtc_get_time(void) {
    ...
    /* Get time components.
       ...
       We repeatedly read the time until it is stable from one read to
       another, in case we start our initial read in the middle of an update.
       This strategy is not recommended by the MC146818A datasheet, but it is
       simpler than any of their suggestions and, furthermore, it is also used
       by Linux.

       The MC146818A can be configured for BCD or binary format, but for
       historical reasons everyone always uses BCD format except on obscure
       non-PC platforms, so we don't bother trying to detect the format in
       use. */
    do {
